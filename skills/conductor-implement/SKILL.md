...
4.  **Select Track:**
    -   **If a track name was provided:**
        ...
        2.  If a unique match is found, immediately prompt the user to confirm the selection without repeating the question elsewhere:
            - **Header:** "Confirm"
            - **Question:** "I found track '<track_description>'. Is this correct?"
            - **Response Type:** yes/no
        3.  If no match is found, or if the match is ambiguous, immediately prompt the user to inform them and request the correct track name without repeating the question elsewhere:
            - **Header:** "Clarify"
    ...
    -   **If no track name was provided (or if the previous step failed):**
        ...
        2.  **If a next track is found:**
            -   Immediately prompt the user to confirm the selection without repeating the question elsewhere:
                - **Header:** "Next Track"
                - **Question:** "No track name provided. Would you like to proceed with the next incomplete track: '<track_description>'?"
                - **Response Type:** yes/no
            -   If confirmed, proceed with this track. Otherwise, immediately prompt the user to request the correct track name without repeating the question elsewhere:
                - **Header:** "Clarify"
...
4.  **Execute Tasks and Update Track Plan:**
    ...
    c. **For Each Task, You MUST:**
        i. **Defer to Workflow:** The **Workflow** file is the **single source of truth** for the entire task lifecycle. You MUST now read and execute the procedures defined in the "Task Workflow" section of the **Workflow** file you have in your context. Follow its steps for implementation, testing, and committing precisely.
           - **CRITICAL:** Every human-in-the-loop interaction, confirmation, or request for feedback mentioned in the **Workflow** (e.g., manual verification plans or guidance on persistent failures) MUST be conducted as direct prompts to the user.
...
2.  **Ask for User Choice:** Immediately prompt the user without repeating the question elsewhere:
...
3.  **Handle User Response:**
    ...
    *   **If user chooses "Delete":**
        i. **CRITICAL WARNING:** Before proceeding, immediately prompt the user for final confirmation without repeating the warning elsewhere:
            - **Header:** "Confirm"
