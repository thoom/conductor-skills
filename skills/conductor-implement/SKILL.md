...
4.  **Select Track:**
    -   **If a track name was provided:**
        ...
        2.  If a unique match is found, immediately prompt the user for confirmation:
            - **Header:** "Confirm"
            - **Question:** "I found track '<track_description>'. Is this correct?"
            - **Type:** yes/no
        3.  If no match is found, or if the match is ambiguous, immediately prompt the user to clarify:
            - **Header:** "Clarify"
    ...
    -   **If no track name was provided (or if the previous step failed):**
        ...
        2.  **If a next track is found:**
            -   Immediately prompt the user for confirmation:
                - **Header:** "Next Track"
                - **Question:** "No track name provided. Would you like to proceed with the next incomplete track: '<track_description>'?"
                - **Type:** yes/no
            -   If confirmed, proceed with this track. Otherwise, immediately prompt the user to provide the correct track name:
                - **Header:** "Clarify"
...
2.  **Update Status to 'In Progress':**
    ...
    -   This requires finding the specific heading for the track and replacing it with the updated status.
...
4.  **Execute Tasks and Update Track Plan:**
    ...
    c. **For Each Task, You MUST:**
        i. **Defer to Workflow:** The **Workflow** file is the **single source of truth** for the entire task lifecycle. You MUST now read and execute the procedures defined in the "Task Workflow" section of the **Workflow** file you have in your context. Follow its steps for implementation, testing, and committing precisely.
           - **CRITICAL:** Every human-in-the-loop interaction, confirmation, or request for feedback mentioned in the **Workflow** (e.g., manual verification plans or guidance on persistent failures) MUST be handled by directly prompting the user.
...
5.  **Finalize Track:**
    ...
    -   This requires finding the specific heading for the track and replacing it with the completed status.
...
5.  **Analyze and Update:**
    ...
    b.  **Update Product Definition:**
        ...
        ii. **Propose and Confirm Changes:** If an update is needed:
            -   **Ask for Approval:** Prompt the user for confirmation, embedding the proposed updates (in diff format) directly into the question.
                - **Header:** "Product"
                - **Question:**
    ...
    d. **Update Product Guidelines (Strictly Controlled):**
        ...
        iii. **Propose and Confirm Changes:** If the conditions are met:
            -   **Ask for Approval:** Prompt the user for confirmation, embedding the proposed changes (in diff format) directly into the question, including a clear warning.
                - **Header:** "Product"
                - **Question:**
...
3.  **Handle User Response:**
    ...
    *   **If user chooses "Archive":**
        ...
        iii. **Remove from Tracks File:** Read the content of the **Tracks Registry** file, remove the entire section for the completed track, and write the modified content back to the file.
    ...
    *   **If user chooses "Delete":**
        i. **CRITICAL WARNING:** Before proceeding, immediately prompt the user for final confirmation:
            - **Header:** "Confirm"
