...
2.  **Handle Failure:** If ANY of these are missing (or their resolved paths do not exist), Announce: "Conductor is not set up. Please run the `conductor-setup` skill." and HALT.
...
1.  **Check for User Input:** First, check if the user provided a track name as an argument (the user-provided arguments passed to this skill).
...
4.  **Select Track:**
    -   **If a track name was provided:**
        ...
        2.  If a unique match is found, prompt the user to confirm the selection (do not repeat the question in the chat):
            > **[Confirm]** "I found track '<track_description>'. Is this correct?" (yes/no)
        3.  If no match is found, or if the match is ambiguous, prompt the user to clarify (do not repeat the question in the chat):
            > **[Clarify]** "I couldn't find a unique track matching the name you provided. Did you mean '<next_available_track>'? Or please type the exact track name." (free text)
    -   **If no track name was provided (or if the previous step failed):**
        ...
        2.  **If a next track is found:**
            -   Prompt the user to confirm the selection (do not repeat the question in the chat):
                > **[Next Track]** "No track name provided. Would you like to proceed with the next incomplete track: '<track_description>'?" (yes/no)
            -   If confirmed, proceed with this track. Otherwise, prompt the user to request the correct track name (do not repeat the question in the chat):
                > **[Clarify]** "Please type the exact name of the track you would like to implement." (free text)
...
4.  **Execute Tasks and Update Track Plan:**
    ...
    c. **For Each Task, You MUST:**
        i. **Defer to Workflow:** The **Workflow** file is the **single source of truth** for the entire task lifecycle. You MUST now read and execute the procedures defined in the "Task Workflow" section of the **Workflow** file you have in your context. Follow its steps for implementation, testing, and committing precisely.
           - **CRITICAL:** Every human-in-the-loop interaction, confirmation, or request for feedback mentioned in the **Workflow** (e.g., manual verification plans or guidance on persistent failures) MUST be communicated to the user via a clear prompt.
...
5.  **Analyze and Update:**
    ...
    b.  **Update Product Definition:**
        ...
        ii. **Propose and Confirm Changes:** If an update is needed:
            -   Prompt the user for approval, embedding the proposed updates (in a diff format) so the user can review them in context (do not repeat the question in the chat):
                > **[Product]** "Please review the proposed updates to the Product Definition below. Do you approve?
                >
                > ---
                >
                > <Insert Proposed product.md Updates/Diff Here>" (yes/no)
    ...
    c.  **Update Tech Stack:**
        ...
        ii. **Propose and Confirm Changes:** If an update is needed:
            -   Prompt the user for approval, embedding the proposed updates (in a diff format) so the user can review them in context (do not repeat the question in the chat):
                > **[Tech Stack]** "Please review the proposed updates to the Tech Stack below. Do you approve?
                >
                > ---
                >
                > <Insert Proposed tech-stack.md Updates/Diff Here>" (yes/no)
    ...
    d. **Update Product Guidelines (Strictly Controlled):**
        ...
        iii. **Propose and Confirm Changes:** If the conditions are met:
            -   Prompt the user for approval, embedding the proposed changes (in a diff format) with a clear warning (do not repeat the question in the chat):
                > **[Product]** "WARNING: This is a sensitive action as it impacts core product guidelines. Please review the proposed changes below. Do you approve these critical changes?
                >
                > ---
                >
                > <Insert Proposed product-guidelines.md Updates/Diff Here>" (yes/no)
...
2.  **Prompt the user for their choice** (do not repeat the question in the chat):
    > **[Track Cleanup]** "Track '<track_description>' is now complete. What would you like to do?"
    >
    > (present the following choices, single select)
    > - **Review** — Run the `conductor-review` skill to verify changes before finalizing.
...
3.  **Handle User Response:**
    *   **If user chooses "Review":**
        *   Announce: "Please run the `conductor-review` skill to verify your changes. You will be able to archive or delete the track after the review."
    ...
    *   **If user chooses "Delete":**
        i. **CRITICAL WARNING:** Before proceeding, prompt the user for final confirmation (do not repeat the warning in the chat):
            > **[Confirm]** "WARNING: This will permanently delete the track folder and all its contents. This action cannot be undone. Are you sure?" (yes/no)
