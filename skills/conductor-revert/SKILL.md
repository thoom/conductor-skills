...
3. **Interaction Paths:**
   ...
   * **PATH A: Direct Confirmation**
     ...
     2. Immediately prompt the user to confirm the selection (do not repeat the question in the chat), using this structure:
        - **questions:**
          - **header:** "Confirm"
          - **question:** "You asked to revert the [Track/Phase/Task]: '[Description]'. Is this correct?"
          - **type:** "yesno"
     3. If "yes", establish this as the `target_intent` and proceed to Phase 2. If "no", immediately prompt the user to answer clarifying questions (do not repeat the question in the chat), using this structure:
        - **questions:**
          - **header:** "Clarify"
   ...
   * **PATH B: Guided Selection Menu**
     ...
     2. **Present a Unified Hierarchical Menu:** Immediately prompt the user to choose from the results (do not list them in the chat first), using this structure:
        - **questions:**
          - **header:** "Select Item"
     ...
     3. **Process User's Choice:**
        ...
        * If the user selects "Other" (automatically added for "choice") or the explicit "Other" option provided, you must engage in a dialogue to find the correct target by prompting the user with a single question of `type: "text"` in the `questions` array.
...
2. **Final Go/No-Go:** Immediately prompt the user for final confirmation (do not repeat the question in the chat), using this structure:
   - **questions:**
     - **header:** "Confirm Plan"
...
3. **Process User Choice:**
   ...
   - If "Revise", immediately prompt the user for the correct plan (do not repeat the question in the chat), using this structure:
     - **questions:**
       - **header:** "Revise"
