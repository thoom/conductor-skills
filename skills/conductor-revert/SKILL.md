...
3.  **Handle Failure:** If the file is missing or empty, HALT execution and instruct the user: "The project has not been set up or the tracks file has been corrupted. Please run the `conductor-setup` skill to set up the plan, or restore the tracks file."
...
2.  **Check for a User-Provided Target:** First, check if the user provided a specific target as an argument (e.g., arguments passed to this skill indicating a track, phase, or task).
...
3.  **Interaction Paths:**
    ...
    *   **PATH A: Direct Confirmation**
        ...
        2.  Prompt the user to confirm the selection (do not repeat the question in the chat):
            > **[Confirm]** "You asked to revert the [Track/Phase/Task]: '[Description]'. Is this correct?" (yes/no)
        3.  If "yes", establish this as the `target_intent` and proceed to Phase 2. If "no", prompt the user for clarification (do not repeat the question in the chat):
            > **[Clarify]** "I'm sorry, I misunderstood. Please describe the Track, Phase, or Task you would like to revert." (free text)
    ...
    *   **PATH B: Guided Selection Menu**
        ...
        2.  **Present a Unified Hierarchical Menu:** Prompt the user with the results (do not list them in the chat first):
            > **[Select Item]** "I found multiple in-progress items (or recently completed items). Please choose which one to revert:"
            >
            > (present the following choices, single select — **CRITICAL:** limit to a maximum of 4 items)
            > - **[Task] <task description>** — Track: <track_id>
            > - **[Phase] <phase description>** — Track: <track_id>
            > - *(etc.)*
        3.  **Process User's Choice:**
            ...
            *   If the user selects "Other" or needs to specify a different target, engage in a free-text dialogue to find the correct target. Once a target is identified, loop back to Path A for final confirmation.
...
2.  **Identify Associated Plan-Update Commits:**
    *   For each validated implementation commit, run `git log` to find the corresponding plan-update commit that happened *after* it and modified the relevant **Implementation Plan** file.
...
3.  **Identify the Track Creation Commit (Track Revert Only):**
    ...
    *   **Method:** Run `git log -- <path_to_tracks_registry>` (resolved via protocol) and search for the commit that first introduced the track entry.
...
2.  **Final Go/No-Go:** Prompt the user for final confirmation (do not repeat the question in the chat):
    > **[Confirm Plan]** "Do you want to proceed with the drafted plan?"
    >
...
3.  **Process User Choice:**
    ...
    - If "Revise", prompt the user to describe the needed changes (do not repeat the question in the chat):
        > **[Revise]** "Please describe the changes needed for the revert plan." (free text)
