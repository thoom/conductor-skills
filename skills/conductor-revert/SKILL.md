...
3. **Interaction Paths:**
   ...
   * **PATH A: Direct Confirmation**
     ...
     2. Immediately prompt the user to confirm the selection with:
        - **Header:** "Confirm"
        - **Question:** "You asked to revert the [Track/Phase/Task]: '[Description]'. Is this correct?"
        - **Type:** yes/no
     3. If "yes", establish this as the `target_intent` and proceed to Phase 2. If "no", immediately prompt the user with clarifying questions:
        - **Header:** "Clarify"
   ...
   * **PATH B: Guided Selection Menu**
     ...
     2. **Present a Unified Hierarchical Menu:** Immediately prompt the user with:
        - **Header:** "Select Item"
        ...
        - **Options:** Provide the identified items as options. Group them by Track in the description if possible. **CRITICAL:** You MUST limit this list to a maximum of 4 items.
          ...
          - **Note:** Include an "Other" path if the selection flow supports it.
     3. **Process User's Choice:**
        ...
        * If the user selects "Other" or an explicit "Other" option, engage in a dialogue to find the correct target using a single free-text prompt.
...
2. **Final Go/No-Go:** Immediately prompt the user for final confirmation with:
   - **Header:** "Confirm Plan"
