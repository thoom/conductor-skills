...
PLAN MODE PROTOCOL: Parts of this process run within a restricted planning mode where only file creation and modification within the `conductor/` directory is permitted. While in this restricted planning mode, you are explicitly permitted and required to create files, edit files, and run authorized shell commands to create and modify files within the `conductor/` directory. **CRITICAL: You MUST use relative paths starting with `conductor/` (e.g., `conductor/product.md`) for all file operations. Do NOT use absolute paths, as they will be blocked by the restricted planning mode's security policies. REDIRECTION (e.g., `>` or `>>`) is strictly NOT allowed in shell commands while in this restricted planning mode and will cause tool failure.**
...
2.  **Get Track Description & Enter Restricted Planning Mode:**
    *   **If the user-provided arguments are empty:**
        1. Enter a restricted planning mode with the reason: "Defining new track".
        2. Prompt the user without repeating the question elsewhere:
            - **Header:** "Description"
            - **Type:** free text
    ...
    *   **If the user-provided arguments contain a description:**
        ...
        2. Enter a restricted planning mode with the reason: "Defining new track".
...
5.  **Write Files:**
    ...
    *   Write the index file to `<Tracks Directory>/<track_id>/index.md` with content:
        ...
        ```
6.  **Exit Restricted Planning Mode:** Exit the restricted planning mode and use the path: `<Tracks Directory>/<track_id>/index.md`.
