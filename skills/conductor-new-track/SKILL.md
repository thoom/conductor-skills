...
PLAN MODE PROTOCOL: Parts of this process run within a restricted planning mode. While in this mode, only file creation and modification within the `conductor/` directory is permitted. **CRITICAL: You MUST use relative paths starting with `conductor/` (e.g., `conductor/product.md`) for all file operations. Do NOT use absolute paths. REDIRECTION (e.g., `>` or `>>`) is strictly NOT allowed in shell commands while in this restricted planning mode.**
...
2.  **Get Track Description & Enter Restricted Planning Mode:**
    *   **If the user-provided arguments are empty:**
        1. Enter a restricted planning mode where only file creation and modification within the `conductor/` directory is permitted. Reason: "Defining new track".
        2. Prompt the user:
            - **Header:** "Description"
            - **Type:** text
    ...
    *   **If the user-provided arguments contain a description:**
        ...
        2. Enter a restricted planning mode where only file creation and modification within the `conductor/` directory is permitted. Reason: "Defining new track".
...
2.  **Questioning Phase:** Ask a series of questions to gather details for the `spec.md`. You must batch up to 4 related questions in a single prompt to streamline the process. Tailor questions based on the track type (Feature or Other).
    ...
    *   **General Guidelines:**
        ...
        *   **2. Formulate the Question:** For each question in the batch, preserve the following structure:
            - **Header:** Very short label (max 16 chars).
            - **Type:** choice, text, or yes/no.
            - **Multi-select:** Required for choice questions. Set to `true` for additive questions or `false` for exclusive choices.
            - **Options:** Required for choice questions. Provide 2-4 options, each with a label and description. Include an "Other" path so the user can provide custom input.
            - **Placeholder:** For text questions, provide a hint.

        *   **3. Interaction Flow:**
            *   Wait for the user's response after each prompt.
            *   If the user selects "Other", use a subsequent text prompt to get their input if necessary.
...
4.  **User Confirmation:**
    -   **Ask for Approval:** Prompt the user for confirmation, embedding the drafted content directly in the question so the user can review it in context.
        - **Header:** "Confirm Spec"
        - **Question:**
...
5.  **Write Files:**
    ...
    *   Write the index file to `<Tracks Directory>/<track_id>/index.md` with content:
        ...
        ```
6.  **Exit Restricted Planning Mode:** Exit restricted planning mode with the path `<Tracks Directory>/<track_id>/index.md`.
