...
PLANNING PHASE PROTOCOL: Parts of this process run within a restricted planning phase. While in this phase, you are explicitly permitted and required to create and modify files within the `conductor/` directory. **CRITICAL: You MUST use relative paths starting with `conductor/` (e.g., `conductor/product.md`) for all file operations. Do NOT use absolute paths. REDIRECTION (e.g., `>` or `>>`) is strictly NOT allowed in shell commands during this phase and will cause tool failure.**
...
2.  **Handle Failure:**
    ...
    -   Announce: "Conductor is not set up. Please run the `conductor-setup` skill to set up the environment."
...
2.  **Get Track Description & Enter Planning Phase:**
    *   **If no arguments were provided by the user:**
        1. **Enter restricted planning mode** (reason: "Defining new track").
        2. Prompt the user (do not repeat the question in the chat):
            > **[Description]** "Please provide a brief description of the track (feature, bug fix, chore, etc.) you wish to start." (free text, e.g., "Implement user authentication")
    ...
    *   **If the user-provided arguments contain a description:**
        ...
        2. **Enter restricted planning mode** (reason: "Defining new track").
...
2.  **Questioning Phase:** Ask a series of questions to gather details for the `spec.md`. You must batch up to 4 related questions in a single prompt to streamline the process. Tailor questions based on the track type (Feature or Other).
    ...
    *   **General Guidelines:**
        ...
        *   **2. Formulate the Question:** Adhere to the following for each question:
            ...
            - For choice questions: indicate whether multi-select (additive) or single-select (exclusive) is required.
            - For choice questions: provide 2-4 options, each with a label and description. Note that an "Other" option for free-text input should be implied.
            - For text questions: provide a hint/placeholder.

        *   **3. Interaction Flow:**
            *   Wait for the user's response after each prompt.
            *   If the user selects "Other", follow up with a free-text prompt to get their input if necessary.
...
4.  **User Confirmation:**
    -   Prompt the user for approval, embedding the drafted content directly so the user can review it in context (do not repeat the question in the chat):
        > **[Confirm Spec]** "Please review the drafted Specification below. Does this accurately capture the requirements?
        >
        > ---
        >
...
3.  **User Confirmation:**
    -   Prompt the user for approval, embedding the drafted content directly so the user can review it in context (do not repeat the question in the chat):
        > **[Confirm Plan]** "Please review the drafted Implementation Plan below. Does this look correct and cover all the necessary steps?
        >
        > ---
        >
...
5.  **Write Files:**
    ...
    *   Write the index file to `<Tracks Directory>/<track_id>/index.md` with content:
        ...
        ```
6.  **Exit restricted planning mode**, declaring the primary output path: `<Tracks Directory>/<track_id>/index.md`.
...
9.  **Announce Completion:** Inform the user:
    > "New track '<track_id>' has been created and added to the tracks file. You can now start implementation by running the `conductor-implement` skill."
