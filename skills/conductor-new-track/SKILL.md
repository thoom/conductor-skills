...
PLAN MODE PROTOCOL: Parts of this process run within a restricted planning mode. While in this mode, you are explicitly permitted and required to create files, edit files, and run authorized shell commands to create and modify files within the `conductor/` directory. **CRITICAL: You MUST use relative paths starting with `conductor/` (e.g., `conductor/product.md`) for all file operations. Do NOT use absolute paths, as they will be blocked by planning-mode security policies. REDIRECTION (e.g., `>` or `>>`) is strictly NOT allowed in shell command calls while in planning mode and will cause tool failure.**
...
2. **Get Track Description & Enter Planning Mode:**
   * **If the user-provided arguments are empty:**
     1. Enter a restricted planning mode where only file creation and modification within the `conductor/` directory is permitted. No destructive or out-of-scope operations. The reason is: "Defining new track".
   ...
   * **If the user-provided arguments contain a description:**
     ...
     2. Enter a restricted planning mode where only file creation and modification within the `conductor/` directory is permitted. No destructive or out-of-scope operations. The reason is: "Defining new track".
...
2. **Questioning Phase:** Prompt the user with a series of questions to gather details for the `spec.md`. You must batch up to 4 related questions in a single prompt to streamline the process. Tailor questions based on the track type (Feature or Other).
   ...
   * **General Guidelines:**
     ...
     * **2. Formulate the Question:** Adhere to the following for each question:
       ...
       - **Multi-select:** For choice questions, set to `true` for multi-select (additive) or `false` for single-choice (exclusive).
       - **Options:** For choice questions, provide 2-4 options, each with a label and description. Note that an "Other" path may be available.
       - **Placeholder:** For free-text questions, provide a hint.

     * **3. Interaction Flow:**
       * Wait for the user's response after each prompt.
       * If the user selects "Other", use a subsequent free-text prompt to get their input if necessary.
...
6. **Exit Planning Mode:** Exit restricted planning mode.
