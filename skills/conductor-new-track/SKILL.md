...
PLAN MODE PROTOCOL: Parts of this process run within a restricted planning mode. While in this mode, you are explicitly permitted and required to create and edit files within the `conductor/` directory. **CRITICAL: You MUST use relative paths starting with `conductor/` (e.g., `conductor/product.md`) for all file operations. Do NOT use absolute paths, as they will be blocked by restricted planning mode policies. REDIRECTION (e.g., `>` or `>>`) is strictly NOT allowed in shell commands while in restricted planning mode and will cause tool failure.**
...
2. **Get Track Description & Enter Restricted Planning Mode:**
   * **If the user-provided arguments are empty:**
     1. Enter a restricted planning mode where only file creation and modification within the `conductor/` directory is permitted. No destructive or out-of-scope operations. Use the reason: "Defining new track".
     2. Prompt the user (do not repeat the question in the chat), using this structure:
        - **questions:**
          - **header:** "Description"
          - **type:** "text"
   ...
   * **If the user-provided arguments contain a description:**
     ...
     2. Enter a restricted planning mode where only file creation and modification within the `conductor/` directory is permitted. No destructive or out-of-scope operations. Use the reason: "Defining new track".
...
2. **Questioning Phase:** Ask a series of questions to gather details for the `spec.md`. You must batch up to 4 related questions in a single prompt to streamline the process. Tailor questions based on the track type (Feature or Other).
   ...
   * **General Guidelines:**
     ...
     * **2. Formulate the Question:** Adhere to the following for each question in the `questions` array:
     ...
     * **3. Interaction Flow:**
       ...
       * If the user selects "Other", use a subsequent prompt with `type: "text"` to get their input if necessary.
...
5. **Write Files:**
   ...
   * Write the index file to `<Tracks Directory>/<track_id>/index.md` with content:
     ...
     ```
6. **Exit Restricted Planning Mode:** Exit restricted planning mode.
