...
1. **Check for User Input:**
   - The user provided the following arguments: the arguments passed to this skill.
...
2. **Auto-Detect Scope:**
   ...
   - If one exists, immediately prompt the user to confirm with:
     - **Header:** "Review Track"
     - **Question:** "Do you want to review the in-progress track '<track_name>'?"
     - **Type:** yes/no
   - If no track is in progress, or user says "no", immediately prompt the user for the scope:
     - **Header:** "Select Scope"
     - **Question:** "What would you like to review?"
     - **Type:** free text
     - **Placeholder:** "Enter track name, or 'current' for uncommitted changes"
3. **Confirm Scope:** Ensure you and the user agree on what is being reviewed by immediately prompting the user with:
