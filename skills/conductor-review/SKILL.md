...
2.  **Auto-Detect Scope:**
    ...
    -   If one exists, immediately prompt the user to confirm without repeating the question elsewhere:
        - **Header:** "Review Track"
        - **Question:** "Do you want to review the in-progress track '<track_name>'?"
        - **Type:** yes/no
    -   If no track is in progress, or user says "no", immediately prompt the user for the scope without repeating the question elsewhere:
        - **Header:** "Select Scope"
        - **Question:** "What would you like to review?"
        - **Type:** free text
        - **Placeholder:** "Enter track name, or 'current' for uncommitted changes"
3.  **Confirm Scope:** Ensure you and the user agree on what is being reviewed by immediately prompting the user without repeating the question elsewhere:
...
3.  **Load and Analyze Changes (Smart Chunking):**
    ...
    -   **Strategy Selection:**
        ...
        -   **Large Changes (> 300 lines):**
            -   **Confirm:** Immediately prompt the user to confirm before proceeding with a large review without repeating the question elsewhere:
                - **Header:** "Large Review"
...
2.  **Action:**
    -   **If issues found:** Immediately prompt the user without repeating the question elsewhere:
        - **Header:** "Decision"
...
2.  **Condition for Action:**
    ...
    -   If changes are detected:
        a. **Check for Track Context:**
            - If you are NOT reviewing a specific track (i.e., you don't have a `plan.md` in context), immediately prompt the user without repeating the question elsewhere:
        ...
        b. **Handle Track-Specific Changes:**
            i.   **Confirm with User:** Immediately prompt the user without repeating the question elsewhere:
                - **Header:** "Commit & Track"
...
3.  **Handle User Response:**
    ...
    *   **If "Delete":**
        i.   **Confirm:** Immediately prompt the user for final confirmation without repeating the warning elsewhere:
            - **Header:** "Confirm"
