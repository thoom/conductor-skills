...
PLAN MODE PROTOCOL: This setup process runs entirely within a restricted planning mode where only file creation and modification within the `conductor/` directory is permitted. While in this restricted planning mode, you are explicitly permitted and required to create files, edit files, and run authorized shell commands to create and modify files within the `conductor/` directory. **CRITICAL: You MUST use relative paths starting with `conductor/` (e.g., `conductor/product.md`) for all file operations. Do NOT use absolute paths, as they will be blocked by the restricted planning mode's security policies. REDIRECTION (e.g., `>` or `>>`) is strictly NOT allowed in shell commands while in this restricted planning mode and will cause tool failure.** Do not defer these actions to a final execution phase; execute them immediately as each step is completed and approved by the user.
...
1.  **Enter Restricted Planning Mode:** Enter a restricted planning mode with the reason: "Setting up Conductor project".
...
5.  **Inquire about Project Goal (for Greenfield):**
    -   **Prompt the user with the following question and wait for their response before proceeding to the next step:**
        - **Header:** "Project Goal"
        - **Type:** free text
...
4.  **Gather Information (Conditional):**
    ...
    -   **If user chose "Interactive":** Use a single prompt to gather detailed requirements (e.g., target users, goals, features).
        ...
        -   **Formulation Guidelines:** Construct the questions so each object has:
            ...
            - **Note:** The "Other" option for custom input should also be available.
...
2.  **Determine Mode:**
    ...
    -   **FOR BROWNFIELD PROJECTS:**
        ...
        -   **Request Confirmation:** After stating the detected stack, you MUST prompt the user for confirmation:
            ...
            - **Type:** yes/no
        -   **Handle Disagreement:** If the user answers 'no' (disputes the suggestion), you MUST immediately prompt the user with a free-text response so they can provide the correct technology stack manually. Once provided, proceed to draft the document using the user's input.
...
2.  **Select Code Style Guides:**
    -   List the available style guides by running a shell command to execute `ls templates/code_styleguides/`. **CRITICAL: You MUST use a shell command for this step. Do NOT use a directory-listing tool for this step.**
    -   **FOR GREENFIELD PROJECTS:**
        ...
        -   **If user chose "Select from Library":**
            ...
            -   **Constraint Handling:** If the final group has only 1 item, you MUST add a second option labeled "None" to satisfy the minimum 2-option requirement.
            -   **Question Structure:**
                - **header:** "Code Style Guide"
                - **type:** "choice"
                - **multiSelect:** `true`
...
5.  **Gather Information (Conditional):**
    ...
    -   **If user chose "Interactive":** Use a single prompt to gather detailed requirements.
        ...
        -   **Formulation Guidelines:** Use "choice" type. Set `multiSelect` to `true` for additive answers. Construct the questions so each object has a `header` (max 16 chars), `question`, and `options` (each with `label` and `description`).
        -   **Note:** Do NOT include an "Autogenerate" option here.
        -   **Interaction Flow:** Wait for the user's response, then proceed to the next step.
...
7.  **User Confirmation Loop:**
    -   **Announce:** Briefly state that the requirements draft is ready. Do NOT repeat the request to "review" or "approve" elsewhere.
...
3.  **Generate Track Artifacts:**
    ...
    d. **Exit Restricted Planning Mode:** Exit the restricted planning mode and use the path: `<Tracks Directory>/<track_id>/index.md`.
