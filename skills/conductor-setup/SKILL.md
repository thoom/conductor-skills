...
PLAN MODE PROTOCOL: This setup process runs entirely within a restricted planning mode. While in this mode, you are explicitly permitted and required to create and edit files within the `conductor/` directory. **CRITICAL: You MUST use relative paths starting with `conductor/` (e.g., `conductor/product.md`) for all file operations. Do NOT use absolute paths, as they will be blocked by restricted planning mode policies. REDIRECTION (e.g., `>` or `>>`) is strictly NOT allowed in shell commands while in restricted planning mode and will cause tool failure.** Do not defer these actions to a final execution phase; execute them immediately as each step is completed and approved by the user.
...
1. **Enter Restricted Planning Mode:** Enter a restricted planning mode where only file creation and modification within the `conductor/` directory is permitted. No destructive or out-of-scope operations. Use the reason: "Setting up Conductor project".
...
- **If Brownfield:**
     ...
     - **Begin Brownfield Project Initialization Protocol:**
       - **1.0 Pre-analysis Confirmation:**
         ...
         2. **Ask for Permission:** Prompt the user to grant permission for a read-only scan to analyze the project, using this structure:
            - **header:** "Permission"
       ...
       - **2.1 File Size and Relevance Triage:**
         1. **Respect Ignore Files:** Before scanning any files, you MUST check for the existence of an agent-specific ignore file and `.gitignore`. If either or both exist, you MUST use their combined patterns to exclude files and directories from your analysis. The agent-specific ignore file should take precedence over `.gitignore` if there are conflicts. This is the primary mechanism for avoiding token-heavy, irrelevant files like `node_modules`.
         ...
         3. **Fallback to Manual Ignores:** ONLY if neither an agent-specific ignore file nor `.gitignore` exist, you should fall back to manually ignoring common directories. Example command: `ls -lR -I 'node_modules' -I '.m2' -I 'build' -I 'dist' -I 'bin' -I 'target' -I '.git' -I '.idea' -I '.vscode'`.
...
2. **Determine Mode:** Prompt the user to choose their preferred workflow, using this structure.
   - **questions:**
     - **header:** "Product"
...
2. **Determine Mode:**
   - **FOR GREENFIELD PROJECTS:** Prompt the user to choose the workflow, using this structure:
     - **questions:**
       - **header:** "Tech Stack"
   ...
   - **FOR BROWNFIELD PROJECTS:**
     ...
     - **Request Confirmation:** After stating the detected stack, you MUST ask the user for confirmation, using this structure:
       - **questions:**
         - **header:** "Tech Stack"
         - **question:** "Is the inferred tech stack (listed above) correct?"
         - **type:** "yesno"
     - **Handle Disagreement:** If the user answers 'no' (disputes the suggestion), you MUST immediately prompt the user with `type: "text"` to allow the user to provide the correct technology stack manually. Once provided, proceed to draft the document using the user's input.
...
2. **Select Code Style Guides:**
   - List the available style guides by running a shell command to execute `ls templates/code_styleguides/`. **CRITICAL: You MUST use a shell command for this step. Do NOT rely on a workspace-limited directory-listing capability that cannot read the templates directory.**
   - **FOR GREENFIELD PROJECTS:**
     ...
     - **Determine Mode:** Prompt the user, using this structure:
       - **questions:**
     ...
     - **If user chose "Select from Library":**
       ...
       - **Single Prompt:** Create one prompt containing a `questions` array with one question per group.
...
3. **Determine Mode:** Prompt the user to choose their preferred workflow, using this structure.
   - **questions:**
     - **header:** "Product Reqs"
...
3. **Confirm Proposal:** Prompt the user to validate the proposal, using this structure:
   - **questions:**
...
4. **Action:**
   ...
   - **If user chose "Suggest changes":**
     - Immediately prompt the user again, using this structure:
       - **header:** "New Track"
       - **type:** "text"
...
3. **Generate Track Artifacts:**
   ...
   d. **Exit Restricted Planning Mode:** Exit restricted planning mode.
