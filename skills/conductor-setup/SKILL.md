...
PLAN MODE PROTOCOL: This setup process runs entirely within a restricted planning mode. While in this mode, only file creation and modification within the `conductor/` directory is permitted. **CRITICAL: You MUST use relative paths starting with `conductor/` (e.g., `conductor/product.md`) for all file operations. Do NOT use absolute paths. REDIRECTION (e.g., `>` or `>>`) is strictly NOT allowed in shell commands while in this restricted planning mode.** Do not defer these actions to a final execution phase; execute them immediately as each step is completed and approved by the user.
...
1.  **Enter Restricted Planning Mode:** Enter a restricted planning mode where only file creation and modification within the `conductor/` directory is permitted. Reason: "Setting up Conductor project".
...
-   **If Brownfield:**
        ...
        -   **Begin Brownfield Project Initialization Protocol:**
            ...
            -   **2.1 File Size and Relevance Triage:**
                ...
                3.  **Fallback to Manual Ignores:** ONLY if neither the agent-specific ignore file nor `.gitignore` exist, you should fall back to manually ignoring common directories. Example command: `ls -lR -I 'node_modules' -I '.m2' -I 'build' -I 'dist' -I 'bin' -I 'target' -I '.git' -I '.idea' -I '.vscode'`.
...
4.  **Gather Information (Conditional):**
    ...
    -   **If user chose "Interactive":** Use a single prompt to gather detailed requirements (e.g., target users, goals, features).
        ...
        -   **Formulation Guidelines:** Construct the questions so each object has:
            ...
            - **Note:** Include an "Other" path for custom input.
...
2.  **Determine Mode:**
    ...
    -   **FOR BROWNFIELD PROJECTS:**
        ...
        -   **Request Confirmation:** After stating the detected stack, you MUST prompt the user:
            - **Questions:**
                - **Header:** "Tech Stack"
                - **Question:** "Is the inferred tech stack (listed above) correct?"
                - **Type:** yes/no
        -   **Handle Disagreement:** If the user answers 'no' (disputes the suggestion), you MUST immediately prompt the user with a text question to allow the user to provide the correct technology stack manually. Once provided, proceed to draft the document using the user's input.
...
2.  **Select Code Style Guides:**
    -   List the available style guides by running `ls templates/code_styleguides/`. **CRITICAL: You MUST use a shell command for this step. Do NOT rely on a workspace-limited directory-listing capability for this instruction.**
    -   **FOR GREENFIELD PROJECTS:**
        ...
        -   **If user chose "Select from Library":**
            ...
            -   **Single Prompt:** Create one user prompt containing a questions array with one question per group.
            -   **Constraint Handling:** If the final group has only 1 item, you MUST add a second option labeled "None" to satisfy the minimum two-option constraint.
            -   **Question Structure:**
                - **Header:** "Code Style Guide"
                - **Type:** choice
                - **Multi-select:** `true`
...
3.  **Generate Track Artifacts:**
    ...
    d. **Exit Restricted Planning Mode:** Exit restricted planning mode with the path `<Tracks Directory>/<track_id>/index.md`.
