...
PLAN MODE PROTOCOL: This setup process runs entirely within Plan Mode. While in Plan Mode, you are explicitly permitted and required to create files and edit files within the `conductor/` directory. **CRITICAL: You MUST use relative paths starting with `conductor/` (e.g., `conductor/product.md`) for all file operations. Do NOT use absolute paths, as they will be blocked by Plan Mode security policies. REDIRECTION (e.g., `>` or `>>`) is strictly NOT allowed in shell commands while in Plan Mode and will cause tool failure.** Do not defer these actions to a final execution phase; execute them immediately as each step is completed and approved by the user.
...
1.  **Enter restricted planning mode** (reason: "Setting up Conductor project").
...
| :--- | :--- | :--- |
| All files in `tracks/<track_id>/` (`spec`, `plan`, `metadata`, `index`) | **HALT** | "The project is already initialized. Use the `conductor-new-track` or `conductor-implement` skill." |
...
1.  **Detect Project Maturity:**
    ...
    -   **Brownfield Indicators:**
        ...
        -   If a `.git` directory exists, run `git status --porcelain`. Ignore changes within the `conductor/` directory. If there are *other* uncommitted changes, it may be Brownfield.
...
-   **If Brownfield:**
        ...
        -   **Begin Brownfield Project Initialization Protocol:**
            -   **1.0 Pre-analysis Confirmation:**
                ...
                2.  **Prompt the user for permission** to perform a read-only scan to analyze the project:
                    > **[Permission]** "A brownfield (existing) project has been detected. May I perform a read-only scan to analyze the project?" (yes/no)
            ...
            -   **2.1 File Size and Relevance Triage:**
                1.  **Respect Ignore Files:** Before scanning any files, you MUST check for the existence of project ignore files (e.g., `.gitignore`) and any agent-specific ignore files. If any exist, you MUST use their combined patterns to exclude files and directories from your analysis. Agent-specific ignore file patterns should take precedence over `.gitignore` if there are conflicts. This is the primary mechanism for avoiding token-heavy, irrelevant files like `node_modules`.
                2.  **Efficiently List Relevant Files:** To list the files for analysis, you MUST use a command that respects the ignore files. For example, run `git ls-files --exclude-standard -co | xargs -n 1 dirname | sort -u` which lists all relevant directories (tracked by Git, plus other non-ignored files) without listing every single file. If Git is not used, construct a `find` command that reads the ignore files and prunes the corresponding paths.
                3.  **Fallback to Manual Ignores:** ONLY if neither an agent-specific ignore file nor `.gitignore` exist, fall back to manually ignoring common directories. Example command: `ls -lR -I 'node_modules' -I '.m2' -I 'build' -I 'dist' -I 'bin' -I 'target' -I '.git' -I '.idea' -I '.vscode'`.
...
4.  **Initialize Git Repository (for Greenfield):**
    -   If a `.git` directory does not exist, run `git init` and report to the user that a new Git repository has been initialized.
...
2.  **Determine Mode:** Prompt the user to choose their preferred workflow:
    > **[Product]** "How would you like to define the product details? Whether you prefer a quick start or a deep dive, both paths lead to a high-quality product guide!"
    >
    > (present the following choices, single select)
...
4.  **Gather Information (Conditional):**
    ...
    -   **If user chose "Interactive":** Use a single prompt to gather detailed requirements (e.g., target users, goals, features).
        ...
        -   **Formulation Guidelines:** Construct the questions where each one has:
            ...
            - The "Other" option for free-text input is implied and should be available to the user.
...
5.  **User Confirmation Loop:**
    -   Prompt the user for approval, embedding the drafted content directly so the user can review it in context:
        > **[Review Draft]** "Please review the drafted Product Guide below. What would you like to do next?
        >
        > ---
        >
...
6.  **Write File:** Once approved, write (append) the generated content to the existing `conductor/product.md` file, preserving the `# Initial Concept` section.
...
2.  **Determine Mode:** Prompt the user to choose their preferred workflow:
    > **[Product]** "How would you like to define the product guidelines? You can hand-pick the style or let me generate a standard set."
    >
    > (present the following choices, single select)
...
5.  **User Confirmation Loop:**
    -   Prompt the user for approval, embedding the drafted content directly so the user can review it in context:
        > **[Review Draft]** "Please review the drafted Product Guidelines below. What would you like to do next?
        >
        > ---
        >
...
2.  **Determine Mode:**
    -   **FOR GREENFIELD PROJECTS:** Prompt the user to choose the workflow:
        > **[Tech Stack]** "How would you like to define the technology stack? I can recommend a proven stack for your goal or you can hand-pick each component."
        >
        > (present the following choices, single select)
    ...
    -   **FOR BROWNFIELD PROJECTS:**
        ...
        -   **Request Confirmation:** After stating the detected stack, prompt the user for confirmation:
            > **[Tech Stack]** "Is the inferred tech stack (listed above) correct?" (yes/no)
        -   **Handle Disagreement:** If the user answers 'no' (disputes the suggestion), you MUST immediately prompt the user with a free-text question to allow them to provide the correct technology stack manually. Once provided, proceed to draft the document using the user's input.
...
3.  **Gather Information (Greenfield Interactive Only):**
    -   **If user chose "Interactive":** Use a single prompt to gather detailed preferences.
        ...
        -   **Formulation Guidelines:** Construct the questions where each one has:
            ...
            - Do NOT include an "Autogenerate" option here.
        -   **Interaction Flow:** Wait for the user's response, then proceed to the next step.
...
5.  **User Confirmation Loop:**
    -   Prompt the user for approval, embedding the drafted content directly so the user can review it in context:
        > **[Review Draft]** "Please review the drafted Tech Stack below. What would you like to do next?
        >
        > ---
        >
...
2.  **Select Code Style Guides:**
    -   List the available style guides by running a shell command to list the contents of `templates/code_styleguides/`. **CRITICAL: You MUST run this shell command to list the available guides.**
    -   **FOR GREENFIELD PROJECTS:**
        ...
        -   **Determine Mode:** Prompt the user:
            > **[Code Style Guide]** "How would you like to proceed with the code style guides?"
            >
        ...
        -   **If user chose "Select from Library":**
            ...
            -   **Action:** Announce "I'll present the available guides in groups. Please select all that apply." Then immediately present the batched questions (do not list the questions in the chat).
            -   **Single Prompt:** Create one prompt containing questions for each group.
            -   **Constraint Handling:** If the final group has only 1 item, you MUST add a second option labeled "None" to satisfy the minimum 2 options requirement.
            -   **Question Structure for each group:**
                - **header:** "Code Style Guide"
                - **type:** "choice" (multi-select: true)
    ...
    -   **FOR BROWNFIELD PROJECTS:**
        ...
        -   **Determine Mode:** Prompt the user:
            > **[Code Style Guide]** "I've identified these guides for your project. Would you like to proceed or add more?"
            >
            > (present the following choices)
            > - **Proceed** — Use the suggested guides.
            > - **Add More** — Select additional guides from the library.
        -   **If user chose "Add More":**
            -   **Action:** Announce "I'll present the additional guides. Please select all that apply." Then immediately present the batched questions (do not list the questions in the chat).
            -   **Method:** Dynamically split the available guides into batches of 4 options max. Create one multi-select question for each batch.

3.  **Action:** Construct and run a shell command to create the directory and copy all selected files from `templates/code_styleguides/` into `conductor/code_styleguides/`. For example: `mkdir -p conductor/code_styleguides && cp templates/code_styleguides/python.md templates/code_styleguides/javascript.md conductor/code_styleguides/`
4.  **Continue:** Immediately proceed to the next section.

### 2.5 Select Workflow (Interactive)
1.  **Copy Initial Workflow:**
    -   Copy `templates/workflow.md` (this repository's templates directory) to `conductor/workflow.md`.
2.  **Determine Mode:** Prompt the user to choose their preferred workflow:
    > **[Workflow]** "Do you want to use the default workflow or customize it? The default includes >80% test coverage and per-task commits."
    >
    > (present the following choices)
...
3.  **Gather Information (Conditional):**
    ...
    -   **If user chose "Customize":**
        a. **Initial Batch:** Use a single prompt to gather primary customizations:
            > (present the following questions)
            > - **[Coverage]** "The default required test code coverage is >80%. What is your preferred percentage?" (free text, e.g., "90")
            > - **[Commits]** "Should I commit changes after each task or after each phase?"
            >   (single select)
            >   - **Per Task** — Commit after every completed task
            >   - **Per Phase** — Commit only after an entire phase is complete
            > - **[Summaries]** "Where should I record task summaries?"
            >   (single select)
            >   - **Git Notes** — Store summaries in Git notes metadata
            >   - **Commit Messages** — Include summaries in the commit message body
        b. **Final Tweak (Second Batch):** Once the first batch is answered, immediately prompt the user a second time to show the result and allow for any additional tweaks:
            > **[Workflow]** "Based on your answers, I will configure the workflow with:
            > - Test Coverage: <User Answer 1>%
            > - Commit Frequency: <User Answer 2>
            > - Summary Storage: <User Answer 3>
            >
            > Is there anything else you'd like to change or add to the workflow? (Leave blank to finish or type your additional requirements)." (free text)

4.  **Action:** Edit `conductor/workflow.md` based on all user answers from both steps.
...
3.  **Determine Mode:** Prompt the user to choose their preferred workflow:
    > **[Product Reqs]** "How would you like to define the product requirements? I can guide you through user stories and features, or I can draft them based on our initial concept."
    >
    > (present the following choices)
...
5.  **Gather Information (Conditional):**
    ...
    -   **If user chose "Interactive":** Use a single prompt to gather detailed requirements.
        ...
        -   **Formulation Guidelines:** Use "choice" type. Set multiSelect to `true` for additive answers. Construct questions each with a header (max 16 chars), question text, and options (each with label and description).
        -   Do NOT include an "Autogenerate" option here.
        -   **Interaction Flow:** Wait for the user's response, then proceed to the next step.
...
7.  **User Confirmation Loop:**
    ...
    -   Prompt the user for approval, embedding the drafted requirements directly so the user can review them:
        > **[Review]** "Please review the drafted Product Requirements below. What would you like to do next?
        >
        > ---
        >
...
3.  **Confirm Proposal:** Prompt the user to validate the proposal:
    > **[Confirm Track]** "To get the project started, I suggest the following track: '<Track Title>'. Do you want to proceed with this track?"
    >
    > (present the following choices, single select)
...
4.  **Action:**
    ...
    -   **If user chose "Suggest changes":**
        -   Immediately prompt the user again:
            > **[New Track]** "Please enter the description for the initial track:" (free text, e.g., "Setup CI/CD pipeline")
...
3.  **Generate Track Artifacts:**
    ...
    d. **Exit restricted planning mode**, declaring the primary output path: `<Tracks Directory>/<track_id>/index.md`.
...
3.  **Next Steps:** Inform the user that they can now begin work by running the `conductor-implement` skill.
